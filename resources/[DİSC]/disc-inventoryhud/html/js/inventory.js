...
function inventorySetup(invOwner, items, money, invTier, envAgirlik, abac) {
    ...
    $('#player-used').html(birincienvagirlik);
...
$(document).ready(function () {
    ...
    $('#inventoryOne, #inventoryTwo').on('mouseenter', '.slot', function () {
        ...
        if (itemData !== undefined) {
			...
			$('#esya-agirlik').html('Birim ağırlığı: (' + itemData.limiti + 'kg)');
			$('#esya-label').html(itemData.label);
			$('#esya-toplam').html('Kapladığı toplam ağırlık: (' + itemData.limiti * itemData.qty + 'kg)');
...
function AddItemToSlot(slot, data) {
    ...
    slot.find('.item-count').html(data.qty + " (" + data.limiti * data.qty + ")");
